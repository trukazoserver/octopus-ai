...
function categorize(subject) {
    ...
    if (/billing|payment|charge|invoice|refund/.test(lower))
        return "billing";
    if (/account|login|password|access|signup/.test(lower))
        return "account";
    if (/bug|error|crash|broken|not working/.test(lower))
        return "technical";
    if (/feature|request|suggestion|improvement/.test(lower))
        return "feature-request";
    if (/cancel|unsubscribe|delete/.test(lower))
        return "cancellation";
    return "general";
}
    ...
    commands: [
        {
            ...
            execute: async (args) => {
                ...
                if (!subject)
                    return "Usage: /ticket-create <subject>";
        ...
        {
            name: "/ticket-list",
            description: "List support tickets. Usage: /ticket-list [--status open|closed|all]",
            execute: async (args) => {
                if (tickets.size === 0)
                ...
                if (filtered.length === 0)
                    return `No ${statusFilter} tickets found.`;
                const priorityIcon = (p) => p === "critical"
                    ? "[!!!]"
                    : p === "high"
                        ? "[!!]"
                        : p === "medium"
                            ...
                            : "[ ]";
                return filtered
                    .map((t) => `${priorityIcon(t.priority)} ${t.id} [${t.status}][${t.category}]\n    ${t.subject}`)
                    .join("\n\n");
            },
        },
        {
