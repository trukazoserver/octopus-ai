...
const plugin = {
    manifest: {
        name: "sales",
        version: "1.0.0",
        description: "CRM tools: contact management, deal pipeline, activity tracking, and revenue forecasting",
        author: "OctopusTeam",
    },
    commands: [
        {
            name: "/contact-add",
            description: "Add a contact. Usage: /contact-add <name> --email <email> --company <company> [--phone <phone>] [--tags tag1,tag2]",
            execute: async (args) => {
                const name = args[0];
                if (!name)
                ...
                const tags = tagsIdx !== -1 && args[tagsIdx + 1]
                    ...
                    : [];
                if (!email)
                    return "Email is required. Use --email <email>";
        ...
        {
            name: "/contact-list",
            description: "List contacts. Usage: /contact-list [--company <company>] [--tag <tag>]",
            execute: async (args) => {
                if (contacts.size === 0)
                ...
                if (companyIdx !== -1 && args[companyIdx + 1]) {
                    ...
                    filtered = filtered.filter((c) => c.company.toLowerCase().includes(company));
                }
                ...
                if (filtered.length === 0)
                    return "No contacts match the filters.";
                return filtered
        ...
        {
            name: "/deal-add",
            description: "Add a deal. Usage: /deal-add <title> --contact <contact-id> --value <amount> [--stage lead|qualified|proposal|negotiation]",
            execute: async (args) => {
                const title = args[0];
                if (!title)
                ...
                const stage = stageIdx !== -1 && args[stageIdx + 1] ? args[stageIdx + 1] : "lead";
                if (!contactId)
        ...
        {
            name: "/deal-list",
            description: "List deals by pipeline stage. Usage: /deal-list [--stage <stage>] [--all]",
            execute: async (args) => {
                if (deals.size === 0)
                    return "No deals. Use /deal-add to create one.";
                ...
                if (filtered.length === 0)
                    return "No deals match the criteria.";
        ...
        {
            name: "/deal-move",
            description: "Move a deal to a new stage. Usage: /deal-move <deal-id> <stage>",
            execute: async (args) => {
                const dealId = args[0];
                const newStage = args[1];
                if (!dealId || !newStage)
                    return "Usage: /deal-move <deal-id> <stage>";
        ...
        {
            name: "/deal-note",
            description: "Add a note to a deal. Usage: /deal-note <deal-id> <note text>",
            execute: async (args) => {
                const dealId = args[0];
        ...
        {
            name: "/forecast",
            description: "Revenue forecast based on pipeline. Usage: /forecast [--weighted]",
            execute: async (args) => {
                if (deals.size === 0)
                    return "No deals in pipeline. Use /deal-add to create one.";
                const weighted = args.includes("--weighted");
                const activeDeals = Array.from(deals.values()).filter((d) => !d.stage.startsWith("closed-"));
                ...
                const byStage = STAGES.filter((s) => !s.startsWith("closed-")).map((stage) => {
                    const stageDeals = activeDeals.filter((d) => d.stage === stage);
                    const total = stageDeals.reduce((sum, d) => sum + d.value, 0);
                    const weightedTotal = stageDeals.reduce((sum, d) => sum + (d.value * d.probability) / 100, 0);
                ...
                const wonDeals = Array.from(deals.values()).filter((d) => d.stage === "closed-won");
                const wonTotal = wonDeals.reduce((sum, d) => sum + d.value, 0);
                const lostDeals = Array.from(deals.values()).filter((d) => d.stage === "closed-lost");
        ...
        },
        {
            ...
            execute: async () => {
                ...
                for (const stage of STAGES) {
                    ...
                    lines.push(`${stage.padEnd(14)} [${bar.padEnd(30)}] ${stageDeals.length} deals | ${formatCurrency(total)}`);
                }
                return lines.join("\n");
            },
        },
        {
            name: "/sales-export",
            description: "Export sales data as JSON. Usage: /sales-export [--file <filepath>]",
            execute: async (args) => {
                await ensureDir();
                const data = {
