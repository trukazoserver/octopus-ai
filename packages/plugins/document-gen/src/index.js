...
function resolveVars(content, varArgs) {
    const varMap = {};
    ...
    return content.replace(/\{\{(\w+)\}\}/g, (_, key) => varMap[key] ?? `{{${key}}}`);
}
...
const plugin = {
    manifest: {
        name: "document-gen",
        version: "1.0.0",
        description: "Document generation with templates, Markdown/HTML export, and custom template creation",
        author: "OctopusTeam",
    },
    commands: [
        {
        ...
        {
            name: "/doc-generate",
            description: "Generate a document from a template. Usage: /doc-generate <template> key1=val1 key2=val2 [--save <filename>]",
            execute: async (args) => {
        ...
        {
            name: "/doc-html",
            description: "Convert markdown to HTML. Usage: /doc-html <markdown-file-or-text> [--save <filename>]",
            execute: async (args) => {
                const input = args[0];
                if (!input)
        ...
        {
            name: "/doc-custom",
            description: "Create a custom template. Usage: /doc-custom <name> --desc <description> --vars key1,key2 --content <markdown-content>",
            execute: async (args) => {
                const name = args[0];
                if (!name)
                ...
                const description = descIdx !== -1 && args[descIdx + 1]
                    ? args[descIdx + 1]
                    : "Custom template";
                const varKeys = varsIdx !== -1 && args[varsIdx + 1]
        ...
        {
            name: "/doc-write",
            description: "Write raw markdown content to a file. Usage: /doc-write <filename> <markdown-content>",
            execute: async (args) => {
        ...
        },
        {
            ...
            execute: async (args) => {
                ...
                if (!filepath)
                    return "Usage: /doc-read <filepath>";
                try {
