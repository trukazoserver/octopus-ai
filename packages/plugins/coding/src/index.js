const plugin = {
    ...
    commands: [
        {
            name: "/refactor",
            description: "Analyze code and suggest refactoring improvements. Usage: /refactor <code or file path>",
            execute: async (args) => {
        ...
        {
            name: "/analyze",
            description: "Analyze code for bugs, style issues, and improvements. Usage: /analyze <code>",
            execute: async (args) => {
        ...
        {
            name: "/debug",
            description: "Analyze code for common bugs and errors. Usage: /debug <code>",
            execute: async (args) => {
                const code = args.join(" ").trim();
                if (!code) {
...
function analyzeCode(code) {
    ...
    const functionMatches = code.match(/(?:function\s+\w+|(?:const|let|var)\s+\w+\s*=\s*(?:async\s+)?(?:\([^)]*\)|[^=])\s*=>|(?:async\s+)?(?:\w+)\s*\([^)]*\)\s*{)/g) ?? [];
    const classMatches = code.match(/class\s+\w+/g) ?? [];
    const importMatches = code.match(/(?:import|require)\s*\(?\s*['"]([^'"]+)['"]/g) ?? [];
    const complexityMarkers = code.match(/(?:if|else|for|while|switch|case|try|catch|&&|\|\||\.?\?)/g) ??
        [];
    return {
        lines: lines.length,
...
function generateRefactorSuggestions(code, analysis) {
    const suggestions = [];
    if (analysis.lines > 50 && analysis.functions <= 1) {
        suggestions.push({
            type: "Extract Function",
            description: "This code block is long. Consider breaking it into smaller, focused functions.",
        });
    }
    if (analysis.complexity > 10) {
        suggestions.push({
            type: "Reduce Complexity",
            description: "High cyclomatic complexity detected. Consider simplifying conditional logic or using early returns.",
        });
    }
    ...
    if (code.includes("console.log") && !code.includes("test")) {
        suggestions.push({
            ...
            description: "Remove console.log statements from production code. Use a proper logging library.",
        });
    }
    return suggestions;
}
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line?.includes("await") &&
            ...
            bugs.push({
                ...
                description: "Using 'await' outside of an async function will cause a syntax error.",
        ...
        if (line?.match(/if\s*\([^)]*=[^=]/)) {
            bugs.push({
                ...
                severity: "warning",
                location: `Line ${lineNum}`,
                description: "Possible accidental assignment (=) instead of comparison (== or ===).",
        ...
        if (line?.match(/\w+\s*\[\s*['"][^'"]+['"]\s*\]/) &&
            ...
            bugs.push({
                ...
                severity: "info",
                location: `Line ${lineNum}`,
                description: "Bracket access without optional chaining may throw if object is null/undefined.",
...
function extractCodeParts(code) {
    const parts = [];
    const funcMatches = code.matchAll(/(?:async\s+)?(?:function\s+(\w+)|(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?\([^)]*\)\s*=>)/g);
...
function calculateMetrics(code) {
    return {
        ...
        functions: (code.match(/(?:function\s+\w+|(?:const|let|var)\s+\w+\s*=\s*(?:async\s+)?(?:\([^)]*\))?\s*=>)/g) ?? []).length,
        classes: (code.match(/class\s+\w+/g) ?? []).length,
        comments: (code.match(/(?:\/\/.*$|\/\*[\s\S]*?\*\/)/gm) ?? []).length,
    };
}
function findDuplicateStrings(code) {
