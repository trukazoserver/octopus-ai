...
function computeStats(records, column) {
    const values = records
        .map((r) => r[column])
        .filter((v) => v !== null && typeof v === "number");
    if (values.length === 0)
        return `Column "${column}" has no numeric values.`;
...
function formatTable(records, maxRows = 20) {
    if (records.length === 0)
        return "No records.";
    const keys = Object.keys(records[0]);
    const colWidths = keys.map((k) => Math.max(k.length, ...records.slice(0, maxRows).map((r) => String(r[k] ?? "null").length)));
...
const plugin = {
    manifest: {
        name: "data",
        version: "1.0.0",
        description: "Data analysis, CSV import/export, statistics, filtering, and SQL-like queries",
        author: "OctopusTeam",
    },
    commands: [
        {
            name: "/data-import",
            description: "Import a CSV file or inline CSV data. Usage: /data-import <name> <csv-data-or-filepath>",
            execute: async (args) => {
                const name = args[0];
                if (!name)
...
${formatTable(records, 5)}`;
        ...
        {
            name: "/data-stats",
            description: "Compute statistics for a numeric column. Usage: /data-stats <dataset> <column>",
            execute: async (args) => {
                const name = args[0];
                const column = args[1];
                if (!name || !column)
                    return "Usage: /data-stats <dataset> <column>";
        ...
        {
            name: "/data-query",
            description: "Query dataset with filters. Usage: /data-query <dataset> [col==val] [col>val] ...",
            execute: async (args) => {
                const name = args[0];
                if (!name)
                ...
                const filtered = filters.length > 0 ? filterRecords(records, filters) : records;
        ...
        {
            name: "/data-export",
            description: "Export dataset as CSV. Usage: /data-export <dataset> [--save <filepath>]",
            execute: async (args) => {
                const name = args[0];
                if (!name)
                    return "Usage: /data-export <dataset> [--save <filepath>]";
                const records = datasets.get(name);
                if (!records)
                    return `Dataset "${name}" not found. Use /data-list to see available datasets.`;
                if (records.length === 0)
        ...
        {
            name: "/data-summary",
            description: "Get a full summary of a dataset with stats for all numeric columns. Usage: /data-summary <dataset>",
            execute: async (args) => {
                const name = args[0];
                if (!name)
                    return "Usage: /data-summary <dataset>";
                const records = datasets.get(name);
                if (!records)
                    return `Dataset "${name}" not found. Use /data-list to see available datasets.`;
                if (records.length === 0)
                ...
                for (const key of keys) {
                    if (types[key] === "number") {
                        ...
                        if (unique.size <= 10) {
                            ...
                            const topVals = vals.reduce((acc, v) => {
        ...
        },
        {
            ...
            description: "Compute Pearson correlation between two numeric columns. Usage: /data-correlation <dataset> <col1> <col2>",
            execute: async (args) => {
                const name = args[0];
                const col1 = args[1];
                const col2 = args[2];
                ...
                const strength = Math.abs(corr) >= 0.8
                    ? "strong"
                    : Math.abs(corr) >= 0.5
                        ? "moderate"
                        : "weak";
