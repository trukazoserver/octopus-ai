...
export function createChannelsCommand(): Command {
	const cmd = new Command("channels").description(
			...
			try {
				...
				for (const [name, ch] of Object.entries(channels)) {
					...
					if (managed) {
						...
						const healthStatus = (
							system.connectionManager as unknown as {
								healthMonitor?: {
									getStatus(
										name: string,
									): { healthy: boolean; latency: number } | undefined;
								};
							}
						).healthMonitor?.getStatus(name);
						if (healthStatus) {
	...
	cmd
		...
		.action(async (channel: string) => {
			try {
				const system = await bootstrap();
				const channels = system.config.channels as Record<
					string,
					{ enabled: boolean }
				>;
				...
				if (!(channel in channels)) {
					console.error(
						chalk.red(
							`Error: Channel '${channel}' not found in configuration.`,
						),
					);
					await system.shutdown();
					process.exit(1);
