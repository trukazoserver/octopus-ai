...
export function createMemoryCommand(): Command {
  const cmd = new Command("memory").description("Manage Octopus AI memory system");
  ...
  cmd
    ...
    .action(async (inputPath: string) => {
      try {
        ...
        console.log(chalk.green(`✓ Indexed ${indexed} chunk(s) from ${filePaths.length} file(s)`));
  ...
  cmd
    ...
    .action(async (query: string) => {
      try {
        ...
        console.log(chalk.cyan(`Found ${items.length} matching item(s), removing...`));
