...
function run(cmd: string, opts?: { timeout?: number }): string {
  try {
    return execSync(cmd, { encoding: "utf-8", timeout: opts?.timeout ?? 15000, stdio: ["pipe", "pipe", "pipe"] }).trim();
  } catch {
    return "";
  }
}

function runShell(cmd: string, opts?: { cwd?: string }): { ok: boolean; output: string } {
...
export function checkPrerequisites(): PrereqResult[] {
  ...
  results.push({
    name: "Node.js",
    passed: major >= 22,
    message: major >= 22 ? `${nodeVersion} (>= 22)` : `${nodeVersion} (se requiere >= 22)`,
  ...
  let pythonVersion = run("python --version") || run("python3 --version");
  results.push({
    name: "Python",
    ...
    autoInstall: isWin
      ? async () => {
          ...
          const r = runShell("winget install Python.Python.3.12 --accept-package-agreements --accept-source-agreements");
          if (r.ok || r.output.includes("Already installed") || r.output.includes("Ya se encuentra instalado")) {
  ...
  if (isWin) {
    ...
    results.push({
      name: "Build Tools (C++)",
      passed: buildToolsOk,
      message: buildToolsOk ? "Visual Studio Build Tools detectado" : "No detectado",
      fixHint: "Se intentará instalar Visual Studio Build Tools via winget",
      autoInstall: async () => {
        console.log(chalk.cyan("    Instalando Visual Studio Build Tools 2022 via winget..."));
        console.log(chalk.gray("    Esto puede tardar varios minutos (descarga ~2GB)..."));
        ...
        if (r.ok || r.output.includes("Successfully installed") || r.output.includes("Already installed") || !r.output.includes("error")) {
          console.log(chalk.green("    Visual Studio Build Tools instalado correctamente"));
          return true;
        }
        console.log(chalk.red(`    Error: ${r.output}`));
        return false;
    ...
    results.push({
      ...
      fixHint: "Ejecuta: sudo apt install build-essential (Debian/Ubuntu) o xcode-select --install (macOS)",
      autoInstall: platform() === "darwin"
        ? async () => {
            console.log(chalk.cyan("    Instalando Command Line Tools..."));
            const r = runShell("xcode-select --install");
            console.log(chalk.yellow("    Sigue las instrucciones en pantalla para completar la instalación"));
            return true;
          }
        : undefined,
    });
...
export function checkNativeBindings(): PrereqResult {
  ...
  if (projectRoot) {
    ...
    if (fs.existsSync(bsPath)) {
      try {
        const prebuilds = fs.readdirSync(path.join(bsPath, "prebuilds"), { recursive: true });
        nativeBindingsOk = prebuilds.length > 0;
      } catch {
        nativeBindingsOk = fs.existsSync(path.join(bsPath, "build", "Release", "better_sqlite3.node"));
      }
    }
  }
  ...
  return {
    ...
    message: nativeBindingsOk ? "Bindings nativos OK" : "Bindings nativos no compilados",
      ...
      try {
        ...
        for (const year of years) {
          ...
          for (const edition of editions) {
            const msBuild = path.join(vsPath, year, edition, "MS", "Current", "Bin", "MSBuild.exe");
...
export function printPrereqResults(results: PrereqResult[]): boolean {
  ...
  for (const result of results) {
    ...
    const detail = result.passed ? chalk.gray(result.message) : chalk.yellow(result.message);
...
export async function autoInstallMissing(results: PrereqResult[]): Promise<boolean> {
