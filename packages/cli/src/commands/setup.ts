...
import { DEFAULT_CONFIG } from "@octopus-ai/core/dist/config/defaults.js";
    ...
    .action(async () => {
      ...
      try {
        ...
        const zhipuKey = await askQuestion(
          rl,
          chalk.yellow("Z.ai / ZhipuAI API Key (proveedor por defecto, Enter para saltar): "),
        );
        ...
        const config: OctopusConfig = JSON.parse(JSON.stringify(DEFAULT_CONFIG));
        if (anthropicKey) {
        ...
        loader.save(config);
        console.log(chalk.green("  ✓ Configuration saved to ~/.octopus/config.json"));
