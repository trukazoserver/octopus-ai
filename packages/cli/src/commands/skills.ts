...
import { SkillImprover } from "@octopus-ai/core";
	...
	cmd
		...
		.action(async (name: string) => {
			...
			try {
				...
				const system = await bootstrap();
				const embedding = await system.embedFn(
	...
	cmd
		...
		.action(async (name: string) => {
			try {
				...
				const usageHistory = await system.skillRegistry.getUsageHistory(
					skill.id,
					100,
				);
				...
				const improver = new SkillImprover(
					...
					system.embedFn,
					{
						triggerOnSuccessRate:
							system.config.skills.improvement.triggerOnSuccessRate,
						...
						abTestMajorChanges:
							system.config.skills.improvement.abTestMajorChanges,
	...
	cmd
		...
		.action(async (name: string) => {
			try {
				...
				console.log(
					chalk.gray(`    Times Used:    ${skill.metrics.timesUsed}`),
				);
				console.log(
					chalk.gray(
						`    Success Rate:  ${(skill.metrics.successRate * 100).toFixed(1)}%`,
					),
				);
				console.log(
					chalk.gray(
						`    Avg Rating:    ${skill.metrics.avgUserRating.toFixed(1)}`,
					),
				);
				console.log(chalk.gray(`    Last Used:     ${skill.metrics.lastUsed}`));
				console.log(
					chalk.gray(`    Improvements:  ${skill.metrics.improvementsCount}`),
				);

				console.log(chalk.white("\n  Quality:"));
				console.log(
					chalk.gray(
						`    Completeness: ${(skill.quality.completeness * 100).toFixed(1)}%`,
					),
				);
				console.log(
					chalk.gray(
						`    Accuracy:     ${(skill.quality.accuracy * 100).toFixed(1)}%`,
					),
				);
				console.log(
					chalk.gray(
						`    Clarity:      ${(skill.quality.clarity * 100).toFixed(1)}%`,
					),
				);
	...
	cmd
		...
		.action(async () => {
			try {
				...
				const totalUses = skills.reduce(
					(sum, s) => sum + s.metrics.timesUsed,
					0,
				);
				const avgSuccess =
					totalSkills > 0
						? skills.reduce((sum, s) => sum + s.metrics.successRate, 0) /
							totalSkills
						: 0;
				const avgQuality =
					totalSkills > 0
				...
				console.log(
					chalk.gray(`  Avg Success:     ${(avgSuccess * 100).toFixed(1)}%`),
				);
				console.log(
					chalk.gray(`  Avg Quality:     ${(avgQuality * 100).toFixed(1)}%`),
				);

				const needingImprovement =
					await system.skillRegistry.findSkillsNeedingImprovement();
				if (needingImprovement.length > 0) {
					...
					for (const s of needingImprovement) {
						console.log(
							chalk.gray(
								`    • ${s.name} (success: ${(s.metrics.successRate * 100).toFixed(0)}%)`,
							),
						);
					}
				}
		...
		.action(async (file: string, opts: { force?: boolean }) => {
			try {
				...
				if (!opts.force) {
					...
					if (data.skill?.name) {
						const existing = await system.skillRegistry.getByName(
							data.skill.name,
						);
						if (existing) {
							console.log(
								chalk.yellow(
									`Skill '${data.skill.name}' already exists. Use --force to overwrite.`,
								),
							);
							await system.shutdown();
							return;
			...
			try {
				...
				if (skills.length === 0) {
					...
					for (const skill of skills) {
						...
						console.log(
							chalk.white(`  ${skill.name}`) + chalk.gray(` v${skill.version}`),
						);
						console.log(chalk.gray(`    ${skill.description}`));
						console.log(
							chalk.gray(
								`    Author: ${skill.author} | Downloads: ${skill.downloads} | Quality: ${quality}%`,
							),
						);
					}
				}
