			...
			try {
				...
				if (plugins.length === 0) {
					...
					for (const plugin of plugins) {
						const commands =
							plugin.commands?.map((c) => c.name).join(", ") || "none";
						console.log(
							chalk.white(`  ${plugin.manifest.name}`) +
								chalk.gray(` v${plugin.manifest.version}`),
						);
						console.log(chalk.gray(`    ${plugin.manifest.description}`));
						console.log(
							chalk.gray(
								`    Author: ${plugin.manifest.author} | Commands: ${commands}`,
							),
						);
					}
				}
			...
			try {
				...
				if (results.plugins.length === 0) {
					...
					for (const plugin of results.plugins) {
						const installed = system.pluginRegistry.get(plugin.name)
						...
						console.log(
							chalk.gray(
								`    Author: ${plugin.author} | Downloads: ${plugin.downloads} | Rating: ${plugin.rating.toFixed(1)}⭐`,
							),
						);
	...
	cmd
		...
		.action(async (name: string, opts: { version?: string }) => {
			try {
				...
				const result = await system.pluginMarketplace.install(
					name,
					opts.version,
				);
		...
		.action(async (name?: string) => {
			try {
				...
				if (name) {
					...
					const updates =
						await system.pluginMarketplace.getInstalledWithUpdates();
					...
					if (withUpdates.length === 0) {
						...
						for (const u of withUpdates) {
							console.log(
								chalk.white(`  ${u.name}: ${u.installed} → ${u.latest}`),
							);
	...
	cmd
		...
		.action(async (name: string) => {
			try {
				...
				if (!info) {
					console.log(
						chalk.yellow(`Plugin '${name}' not found in marketplace`),
					);
					await system.shutdown();
					return;
				}
				...
				console.log(
					...
					chalk.gray(String(info.downloads)),
				);
				console.log(
					chalk.white("  Rating:     "),
					chalk.gray(`${info.rating.toFixed(1)} / 5.0`),
				);
