...
import { ConfigValidator } from "@octopus-ai/core/dist/config/validator.js";
import { createDatabaseAdapter } from "@octopus-ai/core/dist/storage/database.js";
import { expandTildePath } from "@octopus-ai/core/dist/utils/helpers.js";
import { NetworkResolver } from "@octopus-ai/core/dist/connection/network.js";
...
async function runChecks(): Promise<CheckResult[]> {
  ...
  results.push({
    name: "Database",
    passed: dbAccessible,
    message: dbAccessible ? "SQLite database accessible" : "Cannot access database",
  });

  let apiKeyConfig: { hasAnthropic: boolean; hasOpenai: boolean; hasGoogle: boolean; hasZhipu: boolean; hasDeepseek: boolean; hasMistral: boolean; hasXai: boolean } = {
    hasAnthropic: false,
    hasOpenai: false,
    hasGoogle: false,
    hasZhipu: false,
  ...
  try {
    ...
    const anyKey = apiKeyConfig.hasAnthropic || apiKeyConfig.hasOpenai || apiKeyConfig.hasGoogle || apiKeyConfig.hasZhipu || apiKeyConfig.hasDeepseek || apiKeyConfig.hasMistral || apiKeyConfig.hasXai;
  ...
  try {
    ...
    void usage;
    if (config.ai.providers.anthropic.apiKey) reachableProviders.push("anthropic");
  ...
  try {
    ...
    results.push({
      name: "Network",
      passed: networkOk,
      message: networkOk ? "Internet connectivity OK" : "Cannot reach external APIs",
    });
  } catch {
    results.push({
      name: "Network",
  ...
  return new Command("doctor")
    ...
    .action(async () => {
      ...
      if (allPassed) {
        ...
        console.log(
          chalk.yellow.bold("  Some checks failed. Please fix the issues above.\n"),
        );
      }
