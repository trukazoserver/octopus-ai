...
export class PluginMarketplace {
  ...
  constructor(pluginRegistry: PluginRegistry, config?: Partial<MarketplaceConfig>) {
    this.pluginRegistry = pluginRegistry;
    this.config = { ...DEFAULT_MARKETPLACE_CONFIG, ...config };
  }

  async search(query: string, options?: { limit?: number; offset?: number }): Promise<MarketplaceSearchResult> {
  ...
  async list(options?: { category?: string; sort?: string; limit?: number }): Promise<MarketplacePluginInfo[]> {
  ...
  async install(pluginName: string, version?: string): Promise<{ success: boolean; message: string; path?: string }> {
    ...
    if (existing) {
      return { success: false, message: `Plugin '${pluginName}' is already installed (v${existing.manifest.version})` };
    }
    ...
    try {
      ...
      return { success: true, message: `Installed ${pluginName}@${manifest.version}`, path: pluginDir };
    } catch (err) {
      await fs.rm(pluginDir, { recursive: true, force: true }).catch(() => {});
      const message = err instanceof Error ? err.message : String(err);
      return { success: false, message: `Failed to install '${pluginName}': ${message}` };
    }
  }
  ...
  async uninstall(pluginName: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      return { success: false, message: `Failed to uninstall '${pluginName}': ${message}` };
    }
  }

  async update(pluginName: string): Promise<{ success: boolean; message: string; newVersion?: string }> {
    ...
    if (existing.manifest.version === info.version) {
      return { success: false, message: `Plugin '${pluginName}' is already up to date (v${info.version})` };
    }
  ...
  async getInstalledWithUpdates(): Promise<Array<{ name: string; installed: string; latest: string; hasUpdate: boolean }>> {
    const installed = this.pluginRegistry.getAll();
    const results: Array<{ name: string; installed: string; latest: string; hasUpdate: boolean }> = [];
  ...
  private searchLocal(query: string, limit: number): MarketplaceSearchResult {
    ...
    for (const plugin of installed) {
      ...
      const descMatch = plugin.manifest.description.toLowerCase().includes(lowerQuery);
  ...
  private async extractAndValidate(archivePath: string, destDir: string): Promise<PluginManifest> {
    const { createReadStream: _crs } = await import('fs');
