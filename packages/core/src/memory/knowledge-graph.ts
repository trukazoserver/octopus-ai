...
export class KnowledgeGraph {
  ...
  async addNode(id: string, label: string, type: string, properties: Record<string, unknown>): Promise<void> {
    ...
    await this.db.run(
      ...
      [
        id, label, type, embeddingBlob, 1.0, 1, 
  ...
  async addEdge(fromId: string, toId: string, relation: string, weight: number): Promise<void> {
    const edgeId = nanoid();
    await this.db.run(`DELETE FROM memory_associations WHERE source_id = ? AND target_id = ?`, [fromId, toId]);
    await this.db.run(
  ...
  async getNeighbors(nodeId: string, minWeight: number = 0, maxDepth: number = 1): Promise<Array<{ id: string; relation: string; weight: number; path: string[] }>> {
    const results: Array<{ id: string; relation: string; weight: number; path: string[] }> = [];
    ...
    while (queue.length > 0) {
      ...
      const edges = await this.db.all<{ source_id: string; target_id: string; strength: number }>(
