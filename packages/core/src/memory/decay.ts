...
export class MemoryDecayEngine {
  ...
  async applyDecay(): Promise<void> {
    const items = await this.db.all<{ id: string; type: string; importance: number; last_accessed: string }>(
      `SELECT id, type, importance, last_accessed FROM memory_items`
    );
    const now = new Date();

    for (const item of items) {
      const lastAccessed = item.last_accessed ? new Date(item.last_accessed) : new Date();
      ...
      if (newImportance < 0.1) {
        ...
        await this.db.run(`DELETE FROM memory_associations WHERE source_id = ? OR target_id = ?`, [item.id, item.id]);
