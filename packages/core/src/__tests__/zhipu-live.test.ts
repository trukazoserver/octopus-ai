	...
	describe.skipIf(!hasApiKey)(`Z.ai GLM Provider (${mode})`, () => {
		...
		it("should complete a simple chat request", async () => {
			try {
					...
					messages: [
						{
							role: "system",
							content:
								"You are a helpful assistant. Respond in exactly 5 words.",
						},
				...
				expect(
					response.content.length + (response.thinking?.[0]?.text.length ?? 0),
				).toBeGreaterThan(0);
				...
				if (
					...
					console.warn(
						`  [${mode}] Skipped: No credits/quota (${msg.slice(0, 80)})`,
					);
					return;
				}
				throw err;
		...
		it("should handle tool calling", async () => {
			try {
				...
				if (response.toolCalls && response.toolCalls.length > 0) {
					...
					console.log(
						`  [${mode}] Tool call: ${tc.function.name}(${JSON.stringify(args)})`,
					);
				} else {
					console.log(
						`  [${mode}] No tool call (content: "${response.content.slice(0, 80)}")`,
					);
				}
			} catch (err) {
				const msg = err instanceof Error ? err.message : String(err);
			...
			try {
				...
				for await (const chunk of provider.chatStream({
					...
					messages: [
						{
							role: "user",
							content: "Say hello in 3 different languages. Be brief.",
						},
					],
					maxTokens: 200,
					reasoning: { effort: "none" },
