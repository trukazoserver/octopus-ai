...
import { MessageType, createMessage, parseMessage, serializeMessage } from '../transport/protocol.js';

describe('Protocol', () => {
  describe('createMessage', () => {
    it('should create a valid protocol message', () => {
      const msg = createMessage(MessageType.request, 'test-channel', { action: 'ping' });
  ...
  describe('parseMessage', () => {
    ...
    it('should throw on missing required fields', () => {
      expect(() => parseMessage(JSON.stringify({ id: '1' }))).toThrow('Invalid protocol message format');
    });

    it('should throw on wrong field types', () => {
      expect(() =>
        parseMessage(JSON.stringify({ id: 123, type: 'request', channel: 'ch', payload: {}, timestamp: 'now' })),
