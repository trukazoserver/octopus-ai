...
describe('Reasoning - ZhipuProvider directo', () => {
  const provider = new ZhipuProvider({ apiKey: ZAI_API_KEY, mode: 'coding-plan' });
  ...
  it('debe separar reasoning_content como thinking cuando hay contenido', async () => {
    try {
      ...
      if (response.thinking) {
        console.log(`  Thinking (${response.thinking.length} bloques): "${response.thinking[0]!.text.slice(0, 200)}"`);
      }
  ...
  it('debe hacer streaming con thinking separado', async () => {
    try {
      ...
      for await (const chunk of provider.chatStream({
        model: 'glm-4.7',
        messages: [
          { role: 'user', content: 'Explica por qué el cielo es azul, paso a paso.' },
        ],
        maxTokens: 800,
      ...
      if (fullThinking) {
        console.log(`  Stream thinking (${thinkingChunks.length} chunks): "${fullThinking.slice(0, 150)}"`);
      } else {
...
describe('Reasoning - LLMRouter inyección automática', () => {
  it('debe inyectar reasoning desde config thinking=high', async () => {
    ...
    try {
      const response = await router.chat({
        model: 'glm-5.1',
        messages: [
          { role: 'user', content: '¿Cuántos números primos hay entre 1 y 50? Piensa paso a paso.' },
        ],
        maxTokens: 300,
      });
      ...
      if (response.thinking) {
        console.log(`  Router thinking: "${response.thinking[0]!.text.slice(0, 200)}"`);
      }
    } catch (err) {
  ...
  it('debe hacer streaming via router con thinking inyectado', async () => {
    ...
    try {
      ...
      for await (const chunk of router.chatStream({
        model: 'glm-5.1',
        messages: [
          { role: 'user', content: 'Dime 3 datos interesantes sobre los pulpos. Sé breve.' },
        ],
        maxTokens: 300,
      })) {
      ...
      if (thinkingChunks.length > 0) {
        console.log(`  Router stream thinking (${thinkingChunks.length} chunks)`);
      }
...
describe('Reasoning - sin reasoning (effort=none)', () => {
  const provider = new ZhipuProvider({ apiKey: ZAI_API_KEY, mode: 'coding-plan' });
  ...
  it('debe funcionar normalmente sin reasoning', async () => {
    try {
      const response = await provider.chat({
        model: 'glm-5.1',
        messages: [
          { role: 'user', content: 'Di "hola mundo" en español, francés y alemán.' },
        ],
        maxTokens: 100,
...
describe('Reasoning - request manual sin config global', () => {
  const provider = new ZhipuProvider({ apiKey: ZAI_API_KEY, mode: 'coding-plan' });
  ...
  it('debe aceptar reasoning directo en el request', async () => {
    try {
      const response = await provider.chat({
        model: 'glm-5.1',
        messages: [
          { role: 'user', content: 'Si tengo 3 manzanas y doy 1, ¿cuántas me quedan? Razona tu respuesta.' },
        ],
        maxTokens: 200,
        reasoning: {
          effort: 'low',
