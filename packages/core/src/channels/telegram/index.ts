...
export class TelegramChannel implements Channel {
    ...
    constructor(
        ...
        this.bot.on('message:text', (ctx) => {
            ...
            const channelMessage: ChannelMessage = {
                ...
                isGroup,
                replyTo: ctx.msg.reply_to_message ? String(ctx.msg.reply_to_message.message_id) : undefined,
                metadata: {
                    fromId: ctx.from?.id
                }
            };
            ...
            for (const handler of this.messageHandlers) {
                try {
                    ...
                    console.error(`Error in Telegram message handler for channel ${this.id}:`, error);
                }
            }
        });
    }
    ...
    public async send(to: string, content: string, options?: { replyTo?: string }): Promise<string> {
        const msg = await this.bot.api.sendMessage(to, content, {
            reply_parameters: options?.replyTo ? { message_id: parseInt(options.replyTo, 10) } : undefined
        });
