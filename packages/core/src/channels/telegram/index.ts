...
export class TelegramChannel implements Channel {
		...
		this.bot.on("message", async (ctx) => {
			...
			try {
				if (ctx.msg.photo) {
					...
					const saved = await mediaContext.save(
						buffer,
						ctx.msg.voice.mime_type || "audio/ogg",
						"Telegram Voice Note",
					);
					...
					const saved = await mediaContext.save(
						buffer,
						ctx.msg.audio.mime_type || "audio/mpeg",
					...
					const saved = await mediaContext.save(
						buffer,
						ctx.msg.video.mime_type || "video/mp4",
					...
					const saved = await mediaContext.save(
						buffer,
						ctx.msg.document.mime_type || "application/octet-stream",
				...
				console.error(
					`Error downloading media from Telegram for channel ${this.id}:`,
					err,
				);
			}
	...
	private rememberMessage(messageKey: string): boolean {
		...
		if (this.recentMessageKeys.length > TelegramChannel.maxRecentMessages) {
