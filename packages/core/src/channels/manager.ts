...
export class ChannelManager {
        ...
        channel.onMessage((msg: ChannelMessage) => {
            for (const handler of this.messageHandlers) {
                try {
                    ...
                    console.error(`Error in message handler for channel ${channel.id}:`, error);
                }
            }
        });
    ...
    public async send(channelId: string, to: string, content: string, options?: { replyTo?: string }): Promise<string> {
        ...
        try {
            return await this.connectionManager.executeWithRetry(channelId, () => channel.send(to, content, options));
        } catch (error) {
