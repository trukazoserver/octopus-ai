import type { ProviderConfig, LLMRequest, LLMResponse, LLMChunk, LLMToolCall, ReasoningEffort } from "../types.js";
  ...
  async chat(request: LLMRequest): Promise<LLMResponse> {
    ...
    const data = (await response.json()) as {
      ...
      usage: {
        ...
        total_tokens: number;
        completion_tokens_details?: { reasoning_tokens?: number; thoughts_tokens?: number };
      };
    };
    ...
    const thinkingTokens = data.usage?.completion_tokens_details?.thoughts_tokens ?? data.usage?.completion_tokens_details?.reasoning_tokens;
    const hasReasoningContent = !!choice.message.reasoning_content && !!choice.message.content;
    ...
    return {
      ...
      ...(hasReasoningContent ? { thinking: [{ type: "thinking" as const, text: choice.message.reasoning_content! }] } : {}),
      ...
      for (const part of parts) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (delta.delta.reasoning_content) yield { thinking: delta.delta.reasoning_content };
            if (delta.finish_reason) yield { finishReason: delta.finish_reason };
          } catch { continue; }
        }
      }
    }
