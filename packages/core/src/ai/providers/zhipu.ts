import type { ProviderConfig, LLMRequest, LLMResponse, LLMChunk, ThinkingBlock } from "../types.js";
...
export class ZhipuProvider extends BaseLLMProvider {
  ...
  async chat(request: LLMRequest): Promise<LLMResponse> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Z.ai API error (${this.mode}): ${response.status} ${errorText}`);
    }
    ...
    const hasBoth = !!choice.message.content && !!choice.message.reasoning_content;
    const thinking: ThinkingBlock[] | undefined = hasBoth
      ...
      : !!choice.message.reasoning_content && !choice.message.content
        ? [{ type: "thinking", text: choice.message.reasoning_content }]
        : undefined;
    const content = choice.message.content || (thinking ? "" : choice.message.reasoning_content) || "";
