import type { ProviderConfig, LLMRequest, LLMResponse, LLMChunk, LLMMessage, LLMToolCall, ThinkingBlock, ReasoningEffort } from "../types.js";
...
export class AnthropicProvider extends BaseLLMProvider {
  ...
  private toAnthropicMessages(messages: LLMMessage[], includeThinking = false): { system?: string; messages: Array<{ role: "user" | "assistant"; content: string | Array<Record<string, unknown>> }> } {
  ...
  private toAnthropicTools(tools: LLMRequest["tools"]): Array<Record<string, unknown>> | undefined {
  ...
  async chat(request: LLMRequest): Promise<LLMResponse> {
    ...
    const includeThinking = request.reasoning?.effort !== "none" && request.reasoning?.effort !== undefined;
    const { system, messages } = this.toAnthropicMessages(request.messages, includeThinking);
    ...
    const body: Record<string, unknown> = {
      ...
      ...(hasThinking ? {} : request.temperature != null ? { temperature: request.temperature } : {}),
      ...(system ? { system } : {}),
      ...thinkingConfig,
    };
        ...
        for (const line of lines) {
          ...
          try {
            const event = JSON.parse(payload) as {
              type: string;
              delta?: { text?: string; thinking?: string; stop_reason?: string; partial_json?: string };
            ...
            } else if (event.type === "tool_use_chunk" && event.delta?.partial_json) {
              yield {
                toolCalls: {
