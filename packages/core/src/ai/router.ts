import type { LLMRouterConfig, LLMRequest, LLMResponse, LLMChunk, UsageStats, ProviderConfig, ReasoningEffort, ReasoningConfig } from "./types.js";
...
const PROVIDER_REGISTRY: Record<string, {
  ...
  zhipu: {
    ...
    hasCodingPlan: true,
    hasFreeTier: true,
    defaultModels: ["glm-5.1", "glm-5", "glm-5-turbo", "glm-4.7", "glm-4.6", "glm-5v-turbo", "glm-4.6v"],
  },
  openrouter: {
    displayName: "OpenRouter",
    factory: (c) => new OpenAICompatibleProvider({
      ...c,
    ...
    hasCodingPlan: false,
    hasFreeTier: false,
    defaultModels: ["openai/gpt-4.1", "anthropic/claude-sonnet-4-6", "google/gemini-2.5-pro", "meta-llama/llama-3.3-70b-instruct"],
  },
  deepseek: {
  ...
  mistral: {
    ...
    hasCodingPlan: true,
    hasFreeTier: false,
    defaultModels: ["mistral-large-3", "mistral-medium-3-1", "mistral-small-4", "codestral-25-08"],
  },
  xai: {
    ...
    hasCodingPlan: false,
    hasFreeTier: false,
    defaultModels: ["grok-4.20-0309-reasoning", "grok-4.20-0309-non-reasoning", "grok-4-1-fast-reasoning"],
  },
  cohere: {
    displayName: "Cohere",
    ...
    hasCodingPlan: false,
    hasFreeTier: true,
    defaultModels: ["command-a-03-2025", "command-a-vision-07-2025", "command-a-reasoning-08-2025"],
  },
  local: {
...
export class LLMRouter {
      ...
      if (registryEntry) {
        try {
          ...
          if (available) {
            ...
            logger.info(`Provider '${name}' (${registryEntry.displayName}) initialized`);
          } else {
            logger.warn(`Provider '${name}' not available (check API key / connectivity)`);
          }
        } catch (err) {
          logger.warn(`Provider '${name}' failed to initialize: ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }

    if (this.providers.size === 0) {
      logger.warn("No AI providers available. Please configure at least one provider with a valid API key.");
    }
  }
  ...
  private resolveProvider(model: string): { provider: BaseLLMProvider; modelName: string; providerName: string } {
    ...
    if (slashIndex === -1) {
      ...
      if (!defaultProvider) throw new Error(`Default provider "${this.config.default}" not available`);
      return { provider: defaultProvider, modelName: model, providerName: this.config.default };
    }
    ...
    if (!provider) {
      ...
      throw new Error(`Provider "${providerName}" not available. Available: ${available}`);
    }
    return { provider, modelName, providerName };
  }

  private trackUsage(provider: string, usage: { promptTokens: number; completionTokens: number }): void {
  ...
  async chat(request: LLMRequest): Promise<LLMResponse> {
    ...
    const { provider, modelName, providerName } = this.resolveProvider(enriched.model);
    ...
    try {
      ...
      if (this.config.fallback) {
        ...
        if (fallbackProvider) {
          logger.warn(`Provider '${providerName}' failed, falling back to '${this.config.fallback}'`);
