...
export class SkillEvaluator {
  ...
  compareSkills(skillA: Skill, skillB: Skill): {
    winner: "a" | "b" | "tie";
  ...
  private generateRecommendations(
    ...
    if (breakdown.completeness < 0.5) {
      ...
      if (skill.instructions.length < 200) {
        recommendations.push("Expand instructions with more detail for better coverage");
      }
      if (skill.triggerConditions.keywords.length < 3) {
        recommendations.push("Add more trigger keywords for better skill matching");
      }
    }

    if (breakdown.accuracy < 0.7 && skill.metrics.timesUsed > 5) {
      recommendations.push("Success rate is low, consider improvement via SkillImprover");
    }

    if (breakdown.clarity < 0.5) {
      if (!/^##\s/m.test(skill.instructions)) {
        recommendations.push("Add structured sections (## headings) to instructions");
      }
      if (skill.examples.length === 0) {
