...
5. **Design the best possible visual system for this specific topic.** You are NOT limited to a catalog or preset — create a unique, distinctive design that makes this deck look like it was crafted by a professional designer. Consider: a topic-appropriate art direction in 2-3 adjectives; a custom palette with dominant/support/accent roles and exact HEX values chosen to evoke the subject; heading and body fonts that match the mood (use Google Fonts in the HTML); type scale with clear hierarchy; grid/margins/spacing rhythm; one recurring visual motif or pattern; image-generation style prompt; footer, numbering, and citation style. The design direction catalog (Educational, Corporate, Tech, Creative, Editorial, Minimal, Industrial, Natural, Luxury, Retro, Kids) is available as REFERENCE INSPIRATION — you may use a catalog profile as-is, blend two, or create something entirely original. What matters is that the result is visually striking and perfectly matched to the topic. Do NOT default to a generic or safe palette.
...
7. **Generate images or graphics for EVERY slide — no exceptions.** Every single slide must have a visual element that illustrates its specific point. A slide with only text is a failed slide. Plan one visual per slide before writing the HTML:
   a. Classify each slide's visual need: hero image (cover, closing), concept illustration (explains an idea), diagram (process, timeline, hierarchy), data chart (numbers, comparisons), icon (small visual accent), decorative background (atmosphere), or infographic (labeled visual with embedded text).
   b. Pick a consistent art-direction prompt derived from the visual system (style, palette, mood). Reuse it across all images for coherence.
...
12. EVERY slide MUST have a visual element — generated image, illustration, diagram, chart, infographic, or decorative graphic. Zero text-only slides allowed. If a slide discusses a concept, generate an illustration of it. If it shows data, build a chart in HTML/CSS/SVG. If it is a quote, add a portrait or decorative element. If it is a section divider, use a full-bleed image. No exceptions.
