...
export class SkillABTester {
  ...
  async createTest(skillId: string, versionA: string, versionB: string, sampleSize: number): Promise<ABTest> {
  ...
  async recordResult(testId: string, version: string, success: boolean): Promise<void> {
  ...
  async evaluateTest(testId: string): Promise<{ winner?: string; confidence: number }> {
    const test = this.tests.get(testId);
    if (!test) throw new Error(`AB Test ${testId} not found`);
    ...
    if (difference > 0.1 && (aTotal >= Math.min(5, test.sampleSize / 2) && bTotal >= Math.min(5, test.sampleSize / 2))) {
