...
export class SkillLoader {
  ...
  private analyzeTaskNeeds(task: TaskDescription): TaskNeeds {
    ...
    const domainKeywords: Record<string, string[]> = {
      code: ["code", "programming", "function", "class", "api", "debug", "refactor", "implement", "build", "deploy", "test"],
      data: ["data", "database", "query", "sql", "analytics", "transform", "etl", "migration"],
      writing: ["write", "document", "article", "blog", "content", "text", "edit", "proofread", "summarize"],
      research: ["research", "analyze", "investigate", "study", "compare", "evaluate", "review"],
      design: ["design", "ui", "ux", "interface", "layout", "prototype", "wireframe"],
      security: ["security", "auth", "encrypt", "vulnerability", "compliance", "audit"],
    };
    ...
    const needsSkill = complexity >= 3 || keywords.length >= 3 || domains.length >= 1;
  ...
  private determineLevel(
    ...
    const level4 = level3 + skill.examples.length * skill.contextEstimate.perExample;
