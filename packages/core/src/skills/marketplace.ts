...
export class SkillMarketplace {
  ...
  async search(query: string, options?: { limit?: number }): Promise<SharedSkillMetadata[]> {
  ...
  async list(options?: { category?: string; limit?: number }): Promise<SharedSkillMetadata[]> {
  ...
  async exportSkill(skillName: string): Promise<{ success: boolean; message: string; filePath?: string }> {
    ...
    try {
      ...
      return { success: true, message: `Skill exported to ${filePath}`, filePath };
    } catch (err) {
  ...
  async importSkill(filePath: string): Promise<{ success: boolean; message: string; skill?: Skill }> {
    try {
      ...
      if (!exportData.skill) {
        return { success: false, message: 'Invalid skill export file: missing skill data' };
      }
      ...
      if (existing) {
        return { success: false, message: `Skill '${skill.name}' already exists. Use --force to overwrite.` };
      }

      const embedding = await this.embedFn(`${skill.name} ${skill.description} ${skill.instructions}`);
      skill.embedding = embedding;

      await this.skillRegistry.save(skill);

      logger.info(`Skill '${skill.name}' imported successfully`);
      return { success: true, message: `Skill '${skill.name}' v${skill.version} imported`, skill };
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
  ...
  async importFromMarketplace(skillName: string): Promise<{ success: boolean; message: string }> {
    try {
      const url = `${this.config.registryUrl}/api/v1/skills/${encodeURIComponent(skillName)}/download`;
      const exportDir = path.join(os.homedir(), '.octopus', 'cache', 'skill-downloads');
  ...
  async publish(skillName: string): Promise<{ success: boolean; message: string }> {
    const skill = await this.skillRegistry.getByName(skillName);
    if (!skill) {
      return { success: false, message: `Skill '${skillName}' not found` };
    }
    ...
    try {
      ...
      return { success: true, message: `Skill '${skillName}' published to marketplace` };
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
