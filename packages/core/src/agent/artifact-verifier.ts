...
export class ArtifactVerifier {
	...
	async verifyArtifact(artifactId: string): Promise<ArtifactVerificationResult> {
		const artifact = await this.db.get<ArtifactRecord>(
			"SELECT id, url, path, artifact_type, exists_verified FROM agent_workflow_artifacts WHERE id = ?",
			[artifactId],
		);
		if (!artifact) {
			return {
		...
		const { exists, error } = this.checkExistence(artifact);
		const now = new Date().toISOString();

		await this.db.run(
			"UPDATE agent_workflow_artifacts SET exists_verified = ?, verified_at = ?, verification_error = ? WHERE id = ?",
			[exists ? 1 : 0, now, error ?? null, artifactId],
		);

		return { artifactId, exists, verifiedAt: now, error: error ?? null };
	}

	async verifyRunArtifacts(runId: string): Promise<ArtifactVerificationResult[]> {
	...
	async verifyTaskArtifacts(taskId: string): Promise<ArtifactVerificationResult[]> {
		const artifacts = await this.db.all<{ id: string }>(
	...
	private checkExistence(artifact: ArtifactRecord): { exists: boolean; error?: string } {
		if (artifact.path) {
			if (existsSync(artifact.path)) {
				const stat = statSync(artifact.path);
				if (stat.size > 0) return { exists: true };
				return { exists: false, error: `File exists but is empty: ${artifact.path}` };
			}
			return { exists: false, error: `File not found at path: ${artifact.path}` };
		}
		...
		if (artifact.url) {
			...
			if (filename) {
				...
				if (existsSync(filePath)) {
					...
					if (stat.size > 0) return { exists: true };
					return { exists: false, error: `Media file exists but is empty: ${filename}` };
				}
			}
			return { exists: false, error: `Media file not found for URL: ${artifact.url}` };
		}
