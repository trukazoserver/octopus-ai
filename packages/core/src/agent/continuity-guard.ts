 ...
 */
// Media-generation verbs (extraer, generar, concatenar, recortar, ...) are
// included so announced ffmpeg/video workflows (e.g. the video-extension skill)
// are detected as pending actions and forced, instead of being announced and
// then abandoned mid-run.
const ACTION_PROMISE_RE =
	/(lo agrego|lo añado|(?:voy a|proceder[eé] a|procedo a)(?:\s+proceder a)?\s+(?:editar|agreg|añad|modific|cambi|cre|actualiz|buscar|naveg|descarg|ejecut|prob|abr|revis|usar|utiliz|inici|solucion|extraer|extract|gener|recort|concaten|convert|render|comprim|subir|import|anim|procesar|proces)|déjame.*?(?:editar|agreg|añad|modific|buscar|naveg|descarg|ejecut|prob|abr|leer y .*?agreg|leer y .*?añad|extraer|extract|gener|recort|concaten|convert|render|comprim|import|anim|procesar)|actividad actual:\s*(?:iniciando|buscando|navegando|descargando|ejecutando|extrayendo|generando|recortando|concatenando|convirtiendo|renderizando|comprimiendo|importando|procesando)|\biniciando\s+(?:la\s+)?(?:búsqueda|busqueda|navegación|navegacion|descarga|ejecución|ejecucion|extracción|extraccion|generación|generacion|concatenación|concatenacion|conversión|conversion|renderizado|compresión|compresion|importación|importacion|procesamiento)|let me (?:edit|add|update|modify|write|apply|search|browse|download|run|test|open|extract|generate|trim|concat|convert|render|compress|import|process|read.*?(?:and|y).*?(?:edit|add|update|modify|write|apply))|i'?ll (?:edit|add|update|modify|write|apply|search|browse|download|run|test|open|extract|generate|trim|concat|convert|render|compress|import|process)|(?:adding|applying|updating|searching|browsing|downloading|starting|extracting|generating|trimming|concatenating|converting|rendering|compressing|importing|processing) now|(?:primero|ahora|luego|despu[eé]s|finalmente)\s+(?:extraig|gener|recort|concaten|convert|renderiz|comprim|import|anim|proces))/i;
