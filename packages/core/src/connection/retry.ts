export class RetryHandler {
  ...
  constructor(config: { maxAttempts?: number; baseDelay?: number; maxDelay?: number } = {}) {
  ...
  private calculateDelay(attempt: number): number {
    return Math.min(this.baseDelay * Math.pow(2, attempt) * Math.random(), this.maxDelay);
  }
  ...
  isRetryable(error: unknown): boolean {
    ...
    if (message.includes('network') || message.includes('timeout') || message.includes('econnrefused') || message.includes('econnreset')) {
      return true;
    }
