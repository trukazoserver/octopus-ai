...
import type { ConnectionManagerConfig, Endpoint, ProxyConfig } from './types.js';
...
export class ConnectionManager {
  ...
  constructor(config: ConnectionManagerConfig) {
    ...
    this.healthMonitor = new HealthMonitor({ interval: config.healthCheckInterval });
  ...
  registerChannel(channelId: string): ManagedConnection {
    const circuitBreaker = new CircuitBreaker({ threshold: this.config.circuitBreakerThreshold });
    const offlineQueue = new OfflineQueue({ maxSize: this.config.offlineQueueSize });
    const managed: ManagedConnection = { channelId, circuitBreaker, offlineQueue };
    this.channels.set(channelId, managed);
    return managed;
  }

  async executeWithRetry<T>(channelId: string, fn: () => Promise<T>): Promise<T> {
